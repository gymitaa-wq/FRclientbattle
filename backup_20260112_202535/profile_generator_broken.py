...
def generate_random_client_profile() -> str:
    ...
    emails.append(f"Email 43 (Client to Self - After Reddit):\n{'Oh no.' if skepticism > 6 else 'Interesting.'} Reddit says advisors are {reddit_sentiment}. {'Now I'm really skeptical.' if skepticism > 6 else 'Need to be careful.'}")
    ...
    if has_spouse:
        ...
        emails.append(f"Email 45 (Spouse to Client):\nBut what about the disability insurance? And the whole life thing for the {'kids' college fund' if num_children > 0 else 'future'}?")
    else:
    ...
    emails.append(f"Email 46 (Client to {'Spouse' if has_spouse else 'Self'}):\n{'I don't know.' if skepticism > 6 else 'This is confusing.'} The advisor made it sound good, but the internet says it's a bad deal.")
